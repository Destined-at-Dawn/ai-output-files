...
> **适用范围**：所有工作区的 Agent（Claude Code、定时任务等）
...
- Example: `agent/PROJ-234-update-git-sop`
...
- `memory/long-term.md` — 持久知识、用户偏好
- `memory/{YYYY-MM-DD}.md` — 每日任务进展

---

## 阶段性落盘验证（Multi-Phase Task Rule）

**多阶段任务，每阶段完成后立即验证落盘，不等最后一起验。**
...
**为什么**：对话可能截断/超时/压缩。最后一起验 = 可能永远验不了。
**来源**：2026-06-09 架构重构，2 个缺口在最终验证时才发现。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-06-09 | v1.1 | 新增「阶段性落盘验证」规则：多阶段任务每阶段完成后立即验证落盘 |
| 2026-05-18 | v1.0 | 初版，基于《AI Agent 时代的 Git 工作流最佳实践》 |
...
|---------|------|----------------------|
| **分布式认知** | 008-协同智能 §概念1 | AGENT.md=Agent团队的"共享心智模型"——5个工作区5个Agent共用同一份行为规范→行为一致性>90%%。没有AGENT.md=5个Agent各做各的→混乱 |
