...
## Cognitive Design Principles (Cognitive Science References)

| Design Decision | Cognitive Principle | Source |
|----------------|-------------------|--------|
| "Trading data only, no opinions" | WYSIATI defense — prices are what IS, opinions are what we THINK (Thinking Fast and Slow §3) |
| "Multi-signal cross-validation" | Bayesian updating — multiple independent signals reduce uncertainty faster (Algorithms of Life §1) |
| "Label time horizon of each signal" | Temporal discounting — mixing time horizons distorts probability estimates (Thinking Fast and Slow §4) |
| "Structured output template" | Cognitive load management — structured format reduces working memory demand (Cognitive Load Theory §1) |
| "Never conclude from single signal" | Base rate neglect — humans over-weight individual data points (Thinking Fast and Slow §5) |
