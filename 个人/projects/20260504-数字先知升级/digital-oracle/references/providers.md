...
## 认知科学支撑：Provider API速查的认知原理（百大认知书籍）

| 认知机制 | 来源 | 在Provider API速查中的应用 |
|---------|------|------------------------------|
| **外部化参考** | 022-认知神经科学 §concept1 | Provider速查=外部化参考——"API端点/参数/返回格式"→"不需要每次查官方文档→查速查卡→秒级获取" |
| **领域专精** | 023-认知觉醒 §concept1 | Provider速查=领域专精——"对每个数据源的API了如指掌→开发时'不需要停'"→"熟悉=效率×10" |
