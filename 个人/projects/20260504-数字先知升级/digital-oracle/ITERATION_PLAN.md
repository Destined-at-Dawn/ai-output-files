...
## 认知科学支撑：数字先知迭代计划的认知原理（百大认知书籍）

| 认知机制 | 来源 | 在数字先知迭代计划中的应用 |
|---------|------|------------------------------|
| **迭代思维** | 019-敏捷革命 §concept1 | 迭代计划=敏捷迭代——"v1→v2→v3"→"每版有明确的改进目标"→"不是'一步到位'→是'小步快跑'"→"迭代=持续进步→'一步到位'=永不动手" |
| **数据源多样化** | 022-认知神经科学 §concept1 | 多数据源=多样化——"股票+新闻+社交"=三个"信息通道"→"单一通道=偏见→多通道=全面"→"信息多样性=决策质量的函数" |
| **渐进式复杂度** | 025-认知负荷理论 §concept1 | 迭代计划=渐进式复杂度——"先加一个数据源→验证→再加一个"→"每步的认知负荷<工作记忆容量"→"一步加10个数据源=系统崩溃"→"渐进=唯一的安全路径" |
