...
## 认知科学支撑：数字先知SKILL的认知原理（百大认知书籍）

| 认知机制 | 来源 | 在数字先知SKILL中的应用 |
|---------|------|------------------------------|
| **技能定义** | 022-认知神经科学 §concept1 | SKILL.md=技能的"接口文档"——"这个技能做什么/输入什么/输出什么/约束是什么"→"接口清晰→使用者知道怎么调用"→"接口模糊→使用者猜→猜错→失败" |
| **模块化** | 025-认知负荷理论 §concept3 | 技能=模块——"数字先知=独立模块→有明确的输入输出"→"模块化=可替换/可组合/可维护"→"不模块化=牵一发而动全身" |
