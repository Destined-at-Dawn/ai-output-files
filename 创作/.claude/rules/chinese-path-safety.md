...
## ⚠️ write_project_file MCP 工具完全不工作（2026-06-07 实测确认）

**实测结果**：`write_project_file`（MCP 工具）返回"已写入 XX 字符 / XX 字节"但**文件从未被创建**。中英文路径均不落盘。

| 工具 | 中文路径 | ASCII 路径 | 结论 |
|------|---------|-----------|------|
| **Write**（原生工具） | ✅ 落盘 | ✅ 落盘 | **唯一可用** |
| **write_project_file**（MCP） | ❌ 不落盘 | ❌ 不落盘 | **完全不可用** |
| **read_project_file**（MCP） | ✅ 能读 | ✅ 能读 | 可用 |

**铁律**：所有文件写入一律用 **Write** 工具（绝对路径），**禁止**用 `write_project_file` MCP。读取可用 `read_project_file` 或 Read。

**根因**：MCP server 的文件写入实现可能有 bug——返回 success 但实际 I/O 未执行。系统指令中"项目文件操作必须用 write_project_file"的建议在此环境下不成立。

**违规检测信号**：
- [ ] 使用 `write_project_file` 写入任何文件
- [ ] Write 后没有用 Python `os.path.exists()` 或 Read 验证
...
- ❌ "write_project_file 返回成功所以文件写入了" → **实测证明它返回成功但不落盘，中英文路径均不工作**
- ❌ "系统指令说必须用 write_project_file" → 系统指令在此环境下不成立，以实测为准
