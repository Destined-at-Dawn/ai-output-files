...
def extract_content(msg_type, content, compressed):
    ...
    if content:
        s = to_str(content)
        if s: return clean_text(s, msg_type)
...
def get_sender_for_group(raw, real_sid):
    ...
    s = to_str(raw)
    if not s: return f'wxid_{real_sid}'
    if '<?xml' in s:
        for tag in ['username', 'displayname', 'nickname']:
            m = re.search(f'<{tag}><!\[CDATA\[(.*?)\]\]>', s)
...
def export_private(db, uname, cfg):
    ...
    if not cur.fetchone():
        print(f'[SKIP] {display}: table {table} not found')
        return None, 0

    cur = db.execute(f'SELECT create_time, local_type, message_content, compress_content, real_sender_id, status FROM {table} ORDER BY create_time ASC')
    rows = cur.fetchall()
    if not rows:
...
def export_group(db, uname, display):
    ...
    for row in rows:
        ...
        if real_sid == 0 and status == 2:
            sender = '小黎'
        elif real_sid == 0: continue
        else: sender = member_map.get(real_sid, f'wxid_{real_sid}')
        text = extract_content(mtype, content, compressed)
        if not text or not text.strip(): continue
        if ds != last_date:
            lines.extend(['', f'## {ds}', ''])
...
for name, (path, count) in sorted(results.items()):
    ...
    print(f'  {name}: {count} 条, {sz/1024:.0f}KB, {path}')
