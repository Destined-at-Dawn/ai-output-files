
"""
v6b: 精确导出聊天记录到个人区交往目录
修正：列名 create_time/source/message_content/compress_content/real_sender_id/status
"""
import sqlite3, json, re, os, sys, zstandard as zstd

DB_PATH = r'C:\Users\13975\AppData\Local\Temp\wechat-decrypted-wxid_3nvidmluot0a22\message_0.db'
MSG_TABLE = 'Msg_b35297daf9c0eccd6a495ff3cb4f63ec'
TARGET_DIR = r'E:\ai产出文件\牛马\个人\个人\projects\20260425-成长历程\成长历程\交往'
os.makedirs(TARGET_DIR, exist_ok=True)

dctx = zstd.ZstdDecompressor()

# ===== 配置 =====
PRIVATE_CHATS = {
    'wxid_bfgr9dxq2o6m22': {
        'display': 'Camellia',
        'senders': {'2': '小黎', '6': 'Camellia', '7': '[系统]'},
    },
    'wxid_4afm4lnos29o12': {
        'display': '米蛋糕（陈雅婷）',
        'senders': {'2': '小黎', '3026': '陈雅婷', '3': '[系统]'},
    },
    'wxid_iaeun6rhbyb532': {
        'display': '少年Lee（OPC合伙人）',
        'senders': {'2': '小黎', None: '少年Lee'},
    },
}

GROUP_CHATS = {
    '50267796386@chatroom': '青柠同频轻创圈',
    '56820316831@chatroom': '破界青年OPC会员',
    '45375846863@chatroom': 'OPC启动计划1.0',
    '49472124843@chatroom': '破界向上生长基地六群',
    '56989512808@chatroom': '牛马AI灰度测试群',
}

# ===== 工具函数 =====
def decompress_zstd(blob):
    if blob is None: return None
    try:
        return dctx.decompress(blob).decode('utf-8', errors='replace')
    except:
        return None

def clean_text(xml_str, msg_type):
    if not xml_str:
        return ''
    text = xml_str

    # 图片
    if msg_type in (3,):
        md5_match = re.search(r'md5[= "]+([a-f0-9]+)', text, re.IGNORECASE)
        cdn_match = re.search(r'cdnattachurl[= "]+([^"]+)', text, re.IGNORECASE)
        parts = ['[图片]']
        if md5_match: parts.append(f'MD5:{md5_match.group(1)}')
        if cdn_match:
            fn_match = re.search(r'[?&]filename=([^&]+)', cdn_match.group(1))
            if fn_match: parts.append(f'文件:{fn_match.group(1)}')
        return ' '.join(parts)

    # 视频
    if msg_type == 43:
        md5 = re.search(r'md5[= "]+([a-f0-9]+)', text, re.IGNORECASE)
        return f'[视频] MD5:{md5.group(1)}' if md5 else '[视频]'

    # 文件/链接/小程序/appmsg
    if msg_type == 49:
        title = re.search(r'<title>(.*?)</title>', text, re.DOTALL)
        if title:
            return f'[链接] {title.group(1).strip()}'
        desc = re.search(r'<des>(.*?)</des>', text, re.DOTALL)
        if desc:
            return f'[文件] {desc.group(1).strip()}'
        return '[文件/链接]'

    # 语音
    if msg_type == 34:
        return '[语音]'

    # 表情
    if msg_type == 47:
        emoji_md5 = re.search(r'md5[= "]+([a-f0-9]+)', text, re.IGNORECASE)
        return f'[表情]' if not emoji_md5 else f'[表情 {emoji_md5.group(1)[:8]}]'

    # 系统消息
    if msg_type == 10000:
        cdata = re.findall(r'<!\[CDATA\[(.*?)\]\]>', text, re.DOTALL)
        if cdata:
            cleaned = ' '.join(cdata).strip()
            if len(cleaned) > 200:
                cleaned = cleaned[:200] + '...'
            return f'[系统] {cleaned}'
        return '[系统消息]'

    # 红包
    if msg_type in (436207665,):
        type_text = re.search(r'<title>(.*?)</title>', text, re.DOTALL)
        return f'[红包: {type_text.group(1)}]' if type_text else '[红包]'

    # 位置
    if msg_type == 48:
        label = re.search(r'label[= "]+([^"]+)', text, re.IGNORECASE)
        return f'[位置] {label.group(1)}' if label else '[位置]'

    # 引用回复
    if msg_type == 57:
        cdata = re.findall(r'<!\[CDATA\[(.*?)\]\]>', text, re.DOTALL)
        if cdata:
            return f'[引用回复] {" ".join(cdata)[:100]}'
        return '[引用回复]'

    # 名片
    if msg_type == 42:
        return '[名片]'

    # 视频号
    if msg_type == 62:
        return '[视频号]'

    # 直播
    if msg_type == 44:
        return '[直播]'

    # 转账
    if msg_type == 436207745:
        return '[转账]'

    # 文本 - 去掉XML但保留内容
    if text.startswith('<msg>') or text.startswith('<?xml'):
        cdata = re.findall(r'<!\[CDATA\[(.*?)\]\]>', text, re.DOTALL)
        if cdata:
            return ' '.join(cdata).strip()
        # 纯XML标签 - 去标签
        cleaned = re.sub(r'<[^>]+>', '', text)
        if cleaned.strip():
            return cleaned.strip()[:500]

    return text.strip()

def extract_content(msg_type, content, compressed):
    """提取消息文本"""
    # 先尝试解压
    if compressed:
        decompressed = decompress_zstd(compressed)
        if decompressed:
            return clean_text(decompressed, msg_type)

    if content:
        return clean_text(content, msg_type)

    return ''

def get_sender_for_group(msg_content, real_sender_id):
    """从群聊消息XML提取发送者名称"""
    if msg_content is None:
        return f'wxid_{real_sender_id}'

    if isinstance(msg_content, bytes):
        try:
            msg_content = msg_content.decode('utf-8', errors='replace')
        except:
            return f'wxid_{real_sender_id}'

    if isinstance(msg_content, str) and '<?xml' in msg_content:
        # 尝试提取 username
        sender = re.search(r'<username><!\[CDATA\[(.*?)\]\]>', msg_content)
        if sender:
            return sender.group(1)
        # 或者 displayname
        sender = re.search(r'<displayname><!\[CDATA\[(.*?)\]\]>', msg_content)
        if sender:
            return sender.group(1)
        # 或者 nickname
        sender = re.search(r'<nickname><!\[CDATA\[(.*?)\]\]>', msg_content)
        if sender:
            return sender.group(1)

    return f'wxid_{real_sender_id}'

# ===== 主导出 =====

def export_private(db, username, config):
    display = config['display']
    smap = config['senders']
    out = os.path.join(TARGET_DIR, f'{display}.md')

    print(f'[私聊] {display} ...', end=' ')

    cur = db.execute(f'''
        SELECT create_time, local_type, message_content, compress_content, real_sender_id, status, source
        FROM {MSG_TABLE}
        WHERE source = ?
        ORDER BY create_time ASC
    ''', (username,))

    rows = cur.fetchall()
    lines = []
    lines.append(f'# {display}')
    lines.append(f'')
    lines.append(f'> 对象: {display} (ID: {username})')
    lines.append(f'> 消息总数: {len(rows)}')
    from datetime import datetime
    lines.append(f'> 导出时间: {datetime.now().strftime("%Y-%m-%d %H:%M")}')
    lines.append(f'')
    lines.append('---')
    lines.append('')

    last_date = ''
    valid_count = 0

    for row in rows:
        t, mtype, content, compressed, real_sid, status, source = row

        dt = datetime.fromtimestamp(t)
        date_s = dt.strftime('%Y-%m-%d')
        time_s = dt.strftime('%H:%M:%S')

        # 发送者
        skey = str(real_sid) if real_sid else str(status)
        sender = smap.get(skey, f'用户{real_sid or status}')

        # 提取文本
        text = extract_content(mtype, content, compressed)
        if not text or text.strip() == '':
            continue

        # 撤回
        if text and 'revokemsg' in text.lower():
            text = '[消息已撤回]'

        # 日期分隔
        if date_s != last_date:
            lines.append(f'')
            lines.append(f'## {date_s}')
            lines.append(f'')
            last_date = date_s

        lines.append(f'**[{time_s}] {sender}**：{text}')
        valid_count += 1

    lines.append(f'')
    lines.append('---')
    lines.append(f'*共 {valid_count} 条有效消息*')

    with open(out, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))

    sz = os.path.getsize(out)
    print(f'OK: {valid_count}条, {sz/1024:.0f}KB')
    return out, valid_count


def export_group(db, username, display):
    out = os.path.join(TARGET_DIR, f'{display}.md')

    print(f'[群聊] {display} ...', end=' ')

    cur = db.execute(f'''
        SELECT create_time, local_type, message_content, compress_content, real_sender_id, status, source
        FROM {MSG_TABLE}
        WHERE source = ?
        ORDER BY create_time ASC
    ''', (username,))

    rows = cur.fetchall()

    # 先扫一遍建立成员映射
    member_map = {}
    for row in rows:
        t, mtype, content, compressed, real_sid, status, source = row
        raw = decompress_zstd(compressed) if compressed else content
        name = get_sender_for_group(raw, real_sid)
        if real_sid and real_sid != 0:
            member_map[real_sid] = name

    # 特别标记小黎
    for sid, name in member_map.items():
        if 'wxid' not in str(name) and len(str(name)) > 3:
            pass  # already has a good name

    from datetime import datetime
    lines = []
    lines.append(f'# {display}')
    lines.append(f'')
    lines.append(f'> 群聊: {display} (ID: {username})')
    lines.append(f'> 消息总数: {len(rows)}')
    lines.append(f'> 识别成员: {len(member_map)} 人')
    lines.append(f'> 导出时间: {datetime.now().strftime("%Y-%m-%d %H:%M")}')
    lines.append(f'')

    if member_map:
        lines.append('## 群成员')
        lines.append('')
        for sid, name in sorted(member_map.items()):
            lines.append(f'- {name}')
        lines.append('')

    lines.append('---')
    lines.append('')

    last_date = ''
    valid_count = 0

    for row in rows:
        t, mtype, content, compressed, real_sid, status, source = row

        dt = datetime.fromtimestamp(t)
        date_s = dt.strftime('%Y-%m-%d')
        time_s = dt.strftime('%H:%M:%S')

        # 发送者
        sender = member_map.get(real_sid, f'用户{real_sid}')
        # 小黎发出的: real_sender_id=0, status=2
        if real_sid == 0 and status == 2:
            sender = '小黎'
        elif real_sid == 0:
            continue  # 系统消息跳过

        text = extract_content(mtype, content, compressed)
        if not text or text.strip() == '':
            continue

        if date_s != last_date:
            lines.append(f'')
            lines.append(f'## {date_s}')
            lines.append(f'')
            last_date = date_s

        lines.append(f'**[{time_s}] {sender}**：{text}')
        valid_count += 1

    lines.append(f'')
    lines.append('---')
    lines.append(f'*共 {valid_count} 条有效消息*')

    with open(out, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))

    sz = os.path.getsize(out)
    print(f'OK: {valid_count}条, {sz/1024:.0f}KB')
    return out, valid_count


# ===== 执行 =====
db = sqlite3.connect(DB_PATH)
results = {}

for uname, cfg in PRIVATE_CHATS.items():
    try:
        p, c = export_private(db, uname, cfg)
        results[cfg['display']] = (p, c)
    except Exception as e:
        print(f'FAIL: {e}')
        import traceback; traceback.print_exc()

for uname, dname in GROUP_CHATS.items():
    try:
        p, c = export_group(db, uname, dname)
        results[dname] = (p, c)
    except Exception as e:
        print(f'FAIL: {e}')
        import traceback; traceback.print_exc()

db.close()

# 汇总
total = sum(c for _, c in results.values())
print(f'\n===== 导出完成 =====')
for name, (path, count) in results.items():
    print(f'  {name}: {count} 条')
print(f'  总计: {total} 条, {len(results)} 个文件')
print(f'  目录: {TARGET_DIR}')
